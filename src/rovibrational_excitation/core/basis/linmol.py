...
class LinMolBasis(BasisBase):
    ...
    def generate_H0_with_params(
        ...
        **kwargs,
        ...
        omega_rad_pfs = omega * conv if omega is not None else self.omega_rad_pfs
        delta_omega_rad_pfs = (
            delta_omega * conv if delta_omega is not None else self.delta_omega_rad_pfs
        )
        B_rad_pfs = B * conv if B is not None else self.B_rad_pfs
        alpha_rad_pfs = alpha * conv if alpha is not None else self.alpha_rad_pfs

        vterm = self.V_array + 0.5
        jterm = self.J_array * (self.J_array + 1)
        energy_freq = (
            omega_rad_pfs + delta_omega_rad_pfs
        ) * vterm - delta_omega_rad_pfs / 2 * vterm**2
