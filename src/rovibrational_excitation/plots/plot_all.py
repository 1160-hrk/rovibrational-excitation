...
def plot_all(
    *,
    ...
    plt.tight_layout()
    fp = os.path.join(
        figures_dir, f"{filename_prefix}_field_{time.strftime('%Y%m%d_%H%M%S')}.png"
    )
    ...
    if idx_tar is not None and int(idx_tar) >= 0:
        ...
        fp = os.path.join(
            figures_dir,
            f"{filename_prefix}_fidelity_{time.strftime('%Y%m%d_%H%M%S')}.png",
        )
        plt.savefig(fp, dpi=300, bbox_inches="tight")
        plt.show()
    ...
    if do_spectrum:
        try:
            ...
            freq_cm = np.asarray(
                converter.convert_frequency(freq_PHz, "PHz", "cm^-1"), dtype=float
            )
            t_center = t[-1] / 2.0
            E_freq_comp = E_freq * (
                np.exp(1j * 2 * np.pi * freq_PHz * t_center)
            ).reshape((len(freq_PHz), 1))
            ...
            span = 500.0
            fmin = float(max(0.0, omega_center_cm - span))
            fmax = float(omega_center_cm + span)
            mask = (freq_cm >= fmin) & (freq_cm <= fmax)
            freq_p = freq_cm[mask]
            int_x_p = intensity_x[mask]
            int_y_p = intensity_y[mask]
            ...
            ax0.set_title("Field Spectrum (Ex)")
            ax0.set_xlim(fmin, fmax)
            ax1.plot(freq_p, int_y_p, color="tab:green", label="|Ey|²")
            ax1.set_xlabel("Wavenumber (cm$^{-1}$)")
            ax1.set_ylabel("Intensity (a.u.)")
            ax1.set_title("Field Spectrum (Ey)")
            ax1.set_xlim(fmin, fmax)
            plt.tight_layout()
            fp = os.path.join(
                figures_dir,
                f"{filename_prefix}_spectrum_{time.strftime('%Y%m%d_%H%M%S')}.png",
            )
            plt.savefig(fp, dpi=300, bbox_inches="tight")
            plt.show()
    ...
    if do_spectrogram:
        try:
            ...
            fmax = center + span
            T_index = len(t_fs) // 20
            res = spectrogram_fast(
                t_fs,
                Ex,
                T=T_index,
                unit_T="index",
                window_type="hamming",
                step=max(1, T_index // 8),
                N_pad=0,
            )
            if len(res) == 4:
            ...
            freq_cm_full = np.asarray(
                converter.convert_frequency(freq_1fs, "PHz", "cm^-1"), dtype=float
            )
            ...
            fp = os.path.join(
                figures_dir,
                f"{filename_prefix}_spectrogram_{time.strftime('%Y%m%d_%H%M%S')}.png",
            )
            plt.savefig(fp, dpi=300, bbox_inches="tight")
            plt.show()
        except Exception as e:
