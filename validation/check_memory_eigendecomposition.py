...
def check_eigendecomposition_memory(V_max, J_max):
    ...
    print(
        f"基底作成後メモリ: {mem_after_basis:.1f} MB (+{mem_after_basis - mem_start:.1f} MB)"
    )
    ...
    print(
        f"双極子行列作成後メモリ: {mem_after_dipole:.1f} MB (+{mem_after_dipole - mem_after_basis:.1f} MB)"
    )
    ...
    print(
        f"行列組み合わせ後メモリ: {mem_after_combination:.1f} MB (+{mem_after_combination - mem_after_dipole:.1f} MB)"
    )
    ...
    print(
        f"エルミート化後メモリ: {mem_after_hermitian:.1f} MB (+{mem_after_hermitian - mem_after_combination:.1f} MB)"
    )
    ...
    print(
        f"固有値分解後メモリ: {mem_after_eigen:.1f} MB (+{mem_after_eigen - mem_after_hermitian:.1f} MB)"
    )
    ...
    print(
        f"U_H作成後メモリ: {mem_after_uh:.1f} MB (+{mem_after_uh - mem_after_eigen:.1f} MB)"
    )
...
if __name__ == "__main__":
    ...
    for result in results:
        print(
            f"{result['dim']:8d} | {result['total_memory_mb']:10.1f} MB | {result['matrix_size_mb']:8.2f} MB | {result['eigen_memory_mb']:12.1f} MB"
        )
