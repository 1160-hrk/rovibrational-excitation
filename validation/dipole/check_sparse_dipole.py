...
def analyze_sparsity(matrix, name):
    ...
    if sp.issparse(matrix):
        ...
        print(
            f"{name}: {matrix.shape}, スパース率: {sparsity:.1f}%, 非零要素: {nonzero_elements}"
        )
        print(
            f"  メモリサイズ: {matrix.data.nbytes / 1024 / 1024:.4f} MB (dense: {total_elements * 16 / 1024 / 1024:.4f} MB)"
        )
    else:
        total_elements = matrix.shape[0] * matrix.shape[1]
...
def compare_eigendecomposition(V_max, J_max):
    ...
    dipole_dense = LinMolDipoleMatrix(
        basis, mu0=1e-30, potential_type="harmonic", backend="numpy", dense=True
    )
    ...
    print(
        f"Dense 固有値分解メモリ増加: {mem_after_eigen_dense - mem_before_eigen_dense:.1f} MB"
    )
    ...
    dipole_sparse = LinMolDipoleMatrix(
        basis, mu0=1e-30, potential_type="harmonic", backend="numpy", dense=False
    )
    ...
    print(
        f"Sparse 固有値分解メモリ増加: {mem_after_eigen_sparse - mem_before_eigen_sparse:.1f} MB"
    )
    ...
    print("\n--- 比較結果 ---")
    print(
        f"メモリ節約: {(mem_after_eigen_dense - mem_before_eigen_dense) - (mem_after_eigen_sparse - mem_before_eigen_sparse):.1f} MB"
    )
...
if __name__ == "__main__":
    ...
    for result in results:
        ...
        print(
            f"{result['dim']:4d} | {result['dense_memory']:10.1f} MB | {result['sparse_memory']:11.1f} MB | {savings:6.1f} MB | {result['sparse_method']}"
        )
