...
def test_twolevel_units():
    ...
    y_structure_match = np.allclose(
        mu_y_cm / np.max(np.abs(mu_y_cm)), expected_y_structure
    )
...
def test_consistency_with_linmol():
    ...
    dipole_viblad = VibLadderDipoleMatrix(
        basis_viblad, mu0=1.0, units="D", potential_type="harmonic"
    )
