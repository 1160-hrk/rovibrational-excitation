...
import numpy as np
import pytest
from rovibrational_excitation.core._rk4_schrodinger import rk4_schrodinger, _field_to_triplets
...
class TestRK4AdvancedOptions:
    ...
    def test_renormalization_option(self):
        ...
        result_no_renorm = rk4_schrodinger(
            H0, mu_x, mu_y, E_field, E_field, psi0, dt=0.1,
    ...
    def test_stride_sampling(self):
        ...
        result_stride1 = rk4_schrodinger(
            H0, mu_x, mu_y, E_field, E_field, psi0, dt=0.1,
            return_traj=True, stride=1
        )
        
        # ストライド2（2点おき記録）
        result_stride2 = rk4_schrodinger(
            H0, mu_x, mu_y, E_field, E_field, psi0, dt=0.1,
            return_traj=True, stride=2
        )
        
        # ストライド5（5点おき記録）
        result_stride5 = rk4_schrodinger(
            H0, mu_x, mu_y, E_field, E_field, psi0, dt=0.1,
            return_traj=True, stride=5
        )
...
class TestRK4CuPyBackend:
    ...
    def test_cupy_vs_numpy_consistency(self):
        ...
        result_numpy = rk4_schrodinger(
            H0, mu_x, mu_y, E_field, E_field, psi0, dt,
            backend="numpy"
        )
...
class TestRK4LVNEComprehensive:
    ...
    def test_lvne_vs_lvne_traj_consistency(self):
        ...
        # 軌跡版
        result_traj = rk4_lvne_traj(H0, mu_x, mu_y, Ex, Ey, rho0, dt, steps, sample_stride=1)
    ...
    def test_lvne_sample_stride(self):
        ...
        # ストライド1
        result_s1 = rk4_lvne_traj(H0, mu_x, mu_y, Ex, Ey, rho0, dt, steps, sample_stride=1)
        
        # ストライド3
        result_s3 = rk4_lvne_traj(H0, mu_x, mu_y, Ex, Ey, rho0, dt, steps, sample_stride=3)
    ...
    def test_lvne_hermiticity_preservation(self):
        ...
        # 非対角要素を持つ初期密度行列
        rho0 = np.array([[0.5, 0.3+0.2j, 0.1],
                         [0.3-0.2j, 0.3, 0.2+0.1j],
                         [0.1, 0.2-0.1j, 0.2]], dtype=complex)
        dt = 0.05
        steps = 2
        ...
        for i in range(result.shape[0]):
            ...
            assert hermitian_error < 1e-12, f"非エルミート at step {i}: {hermitian_error}"
...
class TestRK4EdgeCases:
    ...
    def test_zero_electric_field(self):
        ...
        result = rk4_schrodinger(H0, mu_x, mu_y, E_zero, E_zero, psi0, dt, return_traj=True)
    ...
    def test_very_small_timestep(self):
        ...
        # 小さなdtでは変化が小さいことを確認
        population_change = np.abs(np.abs(result_small[0, 1])**2 - 0.0)  # 初期は基底状態
