...
def test_nondimensionalization_unit_options():
    ...
    # エネルギー単位のH0
    H0_energy = basis.generate_H0_with_params(
        omega=0.159, input_units="rad/fs", units="J"
    )

    # 周波数単位のH0
    H0_freq = basis.generate_H0_with_params(
        omega=0.159, input_units="rad/fs", units="rad/fs"
    )
    ...
    (_, _, _, _, _, _, scales_energy) = nondimensionalize_system(
        H0_energy.matrix, mu_x, mu_y, efield, H0_units="energy", time_units="fs"
    )

    # 周波数単位での無次元化
    (_, _, _, _, _, _, scales_freq) = nondimensionalize_system(
        H0_freq.matrix, mu_x, mu_y, efield, H0_units="frequency", time_units="fs"
    )
...
def test_physical_regime_analysis():
    ...
    _, _, _, _, _, _, scales_weak = nondimensionalize_system(
        H0.matrix, dip.mu_x, dip.mu_y, efield_weak, H0_units="energy", time_units="fs"
    )
    regime_weak = analyze_regime(scales_weak)

    # 強結合解析
    _, _, _, _, _, _, scales_strong = nondimensionalize_system(
        H0.matrix, dip.mu_x, dip.mu_y, efield_strong, H0_units="energy", time_units="fs"
    )
