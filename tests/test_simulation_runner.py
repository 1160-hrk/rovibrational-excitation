...
class TestCheckpointManager:
    ...
    def test_case_hash(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            case2 = {"V_max": 5, "J_max": 3, "amplitude": 0.1, "outdir": "/tmp/different"}
    ...
    def test_save_and_load_checkpoint(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            failed_cases = [
                {"V_max": 10, "J_max": 5, "error": "Out of memory"}
            ]
            total_cases = 10
            start_time = 1234567890.0
            
            # 保存
            manager.save_checkpoint(completed_cases, failed_cases, total_cases, start_time)
...
class TestRunAllWithCheckpoint:
    ...
    def test_checkpoint_creation(self, mock_run_one):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('rovibrational_excitation.simulation.runner._make_root') as mock_make_root:
...
class TestResumeRun:
    ...
    def test_resume_without_checkpoint(self):
        """チェックポイントなしでの再開テスト"""
        with tempfile.TemporaryDirectory() as temp_dir:
            with pytest.raises(ValueError, match="再開可能なチェックポイントが見つかりません"):
                resume_run(temp_dir)
