...
class TestNondimensionalConsistency:
    ...
    def create_test_field(self, duration=50, amplitude=1e9):
        ...
        field_intervals = int(round((tf - ti) / dt4Efield))
        time4Efield = np.linspace(ti, tf, field_intervals + 1)
    ...
    def test_final_state_consistency(self):
        ...
        # NaN値のチェック
        assert not np.any(np.isnan(psi_final_dimensional)), (
            "NaN values in dimensional result"
        )
        assert not np.any(np.isnan(psi_final_nondimensional)), (
            "NaN values in nondimensional result"
        )
        ...
        assert abs(norm_dimensional - 1.0) < 1e-10, (
            f"次元あり系の規格化エラー: {norm_dimensional}"
        )
        assert abs(norm_nondimensional - 1.0) < 1e-10, (
            f"無次元化系の規格化エラー: {norm_nondimensional}"
        )
    ...
    def test_trajectory_consistency(self):
        ...
        assert np.all(np.abs(norm_dimensional - 1.0) < 1e-8), (
            "次元あり系の規格化が保存されていません"
        )
        assert np.all(np.abs(norm_nondimensional - 1.0) < 1e-8), (
            "無次元化系の規格化が保存されていません"
        )
    ...
    def test_weak_field_consistency(self):
        ...
        assert ground_prob_dimensional > 0.5, (
            f"弱電場で基底状態の存在確率が低すぎます: {ground_prob_dimensional}"
        )
        assert ground_prob_nondimensional > 0.5, (
            f"弱電場で基底状態の存在確率が低すぎます: {ground_prob_nondimensional}"
        )
