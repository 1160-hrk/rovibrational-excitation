...
def create_complex_polarization(amplitude=1.0, phase=0.0):
    """複素偏光を生成"""
    pol = np.array(
        [amplitude * np.cos(phase), amplitude * np.sin(phase)], dtype=complex
    )
...
class TestSplitOperatorAdvanced:
    ...
    def test_hermitian_interaction_enforcement(self):
        ...
        traj = splitop_schrodinger(H0, mu_x, mu_y, pol, Efield, psi0, dt=0.1, steps=2)
    ...
    def test_multilevel_system_dynamics(self):
        ...
        traj = splitop_schrodinger(H0, mu_x, mu_y, pol, Efield, psi0, dt=0.05, steps=10)
    ...
    def test_complex_polarization_effects(self):
        ...
        for pol in polarizations:
            traj = splitop_schrodinger(
                H0, mu_x, mu_y, pol, Efield, psi0, dt=0.1, steps=5
            )
    ...
    def test_cupy_large_system(self):
        ...
        traj_numpy = splitop_schrodinger(
            H0, mu_x, mu_y, pol, Efield, psi0, dt=0.1, steps=10, backend="numpy"
        )

        # CuPy版
        traj_cupy = splitop_schrodinger(
            H0, mu_x, mu_y, pol, Efield, psi0, dt=0.1, steps=10, backend="cupy"
        )
    ...
    def test_long_time_stability(self):
        ...
        traj = splitop_schrodinger(
            H0, mu_x, mu_y, pol, Efield, psi0, dt=0.1, steps=n_steps
        )
