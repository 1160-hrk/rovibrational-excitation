...
def make_simple_case():
    ...
    field_x = np.concatenate([np.zeros(5), np.linspace(0, 1, 11), np.zeros(5)])
    field_y = np.zeros_like(field_x)
    psi0 = np.array([1.0, 0.0], dtype=np.complex128)
    dt = 0.1
    return H0, mu_x, mu_y, field_x, field_y, psi0, dt


def test_splitop_schrodinger_norm():
    H0, mu_x, mu_y, field_x, field_y, psi0, dt = make_simple_case()
    traj = splitop_schrodinger(
        H0, mu_x, mu_y, field_x, field_y, psi0, dt, return_traj=True
    )
...
def test_splitop_schrodinger_stride():
    H0, mu_x, mu_y, field_x, field_y, psi0, dt = make_simple_case()
    traj = splitop_schrodinger(
        H0, mu_x, mu_y, field_x, field_y, psi0, dt, return_traj=True, sample_stride=1
    )
    traj2 = splitop_schrodinger(
        H0, mu_x, mu_y, field_x, field_y, psi0, dt, return_traj=True, sample_stride=2
    )
...
def test_splitop_schrodinger_return_final():
    ...
    final = splitop_schrodinger(
        H0, mu_x, mu_y, field_x, field_y, psi0, dt, return_traj=False
    )
