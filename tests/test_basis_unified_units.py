...
def test_twolevel_basis_units():
    ...
    assert H0_energy.matrix[1, 1] == pytest.approx(energy_gap_J)

    # 周波数単位の入力
    freq_gap_rad_pfs = 0.159  # rad/fs
    basis_from_freq = TwoLevelBasis(
        energy_gap=freq_gap_rad_pfs, input_units="rad/fs", output_units="J"
    )
    H0_from_freq = basis_from_freq.generate_H0()

    # 周波数単位で出力
    basis_freq_out = TwoLevelBasis(
        energy_gap=energy_gap_J, input_units="J", output_units="rad/fs"
    )
...
def test_viblad_basis_units():
    ...
    basis = VibLadderBasis(
        V_max=3, omega=0.159, delta_omega=0.001, input_units="rad/fs", output_units="J"
    )
...
def test_all_basis_consistency():
    ...
    # VibLadderBasis
    viblad = VibLadderBasis(
        V_max=1, omega=omega, input_units="rad/fs", output_units="J", delta_omega=0.0
    )
...
def test_backward_compatibility():
    ...
    # TwoLevelBasis
    twolevel = TwoLevelBasis(energy_gap=1.0)
    H0_old = twolevel.generate_H0_with_params(
        energy_gap=1.0, energy_gap_units="rad/fs", units="J"
    )
    ...
    # VibLadderBasis
    viblad = VibLadderBasis(V_max=2, omega=1.0, delta_omega=0.0)
    H0_old = viblad.generate_H0_with_params(
        omega=0.159, input_units="rad/fs", units="J"
    )
    viblad_new = VibLadderBasis(
        V_max=2, omega=0.159, input_units="rad/fs", output_units="J", delta_omega=0.0
    )
