...
from rovibrational_excitation.core.electric_field import (
    ElectricField, gaussian, gaussian_fwhm, lorentzian, lorentzian_fwhm, 
    voigt, voigt_fwhm, apply_dispersion
)
...
def test_electricfield_multiple_pulses():
    ...
    ef.add_dispersed_Efield(
        gaussian_fwhm, duration=2.0, t_center=-2.0, 
    ...
    ef.add_dispersed_Efield(
        gaussian_fwhm, duration=2.0, t_center=2.0,
        carrier_freq=1.0, amplitude=0.5,
        polarization=np.array([1.0, 0.0]), const_polarisation=True
    )
...
def test_electricfield_dispersion():
    ...
    ef.add_dispersed_Efield(
        ...
        carrier_freq=1.0, amplitude=1.0,
        polarization=np.array([1.0, 0.0]),
        gdd=0.1, tod=0.01, const_polarisation=True
    )
...
def test_electricfield_modulation():
    ...
    ef.apply_sinusoidal_mod(
        center_freq=1.0, amplitude=0.1, carrier_freq=0.5,
        phase_rad=0.0, type_mod="phase"
    )
...
def test_electricfield_spectrum_analysis():
    ...
    ef.add_dispersed_Efield(
        ...
        carrier_freq=carrier_freq, amplitude=1.0,
        polarization=np.array([1.0, 0.0]), const_polarisation=True
    )
...
def test_electricfield_edge_cases():
    ...
    ef_zero.add_dispersed_Efield(
        gaussian_fwhm, duration=1.0, t_center=0.0,
        carrier_freq=1.0, amplitude=0.0,
        polarization=np.array([1.0, 0.0]), const_polarisation=True
    )
    # ゼロ振幅でも偏光情報は設定される
