...
class TestRK4SchrodingerDetailed:
    ...
    def test_time_reversal_symmetry(self):
        ...
        # 前進伝播
        traj_forward = rk4_schrodinger(H0, mu_x, mu_y, Ex, Ey, psi0, dt, return_traj=True)
        ...
        traj_backward = rk4_schrodinger(H0, mu_x, mu_y, Ex_rev, Ey_rev, psi_end, -dt, return_traj=True)
    ...
    def test_backend_consistency(self):
        ...
        # NumPy
        result_numpy = rk4_schrodinger(H0, mu_x, mu_y, Ex, Ey, psi0, dt=0.1, 
                                     return_traj=False, backend="numpy")
        
        # CuPy
        result_cupy = rk4_schrodinger(H0, mu_x, mu_y, Ex, Ey, psi0, dt=0.1,
                                    return_traj=False, backend="cupy")
    ...
    def test_renormalization_option(self):
        ...
        # 再正規化あり
        traj_renorm = rk4_schrodinger(H0, mu_x, mu_y, Ex, Ey, psi0, dt=0.1,
                                    return_traj=True, renorm=True)
        
        # 再正規化なし
        traj_no_renorm = rk4_schrodinger(H0, mu_x, mu_y, Ex, Ey, psi0, dt=0.1,
                                       return_traj=True, renorm=False)
...
class TestRK4LiouvilleVonNeumannDetailed:
    ...
    def test_trace_conservation_precision(self):
        ...
        traj = rk4_lvne_traj(H0, mu_x, mu_y, Ex, Ey, rho0, dt=0.05, steps=10, sample_stride=1)
        
        initial_trace = np.trace(rho0)
        
        for i, rho in enumerate(traj):
            trace = np.trace(rho)
            np.testing.assert_allclose(trace, initial_trace, atol=1e-12,
                                     err_msg=f"Step {i}: trace = {trace}")
    ...
    def test_hermiticity_preservation(self):
        ...
        # 初期エルミート密度行列
        rho0 = np.array([[0.5, 0.1+0.2j, 0.0], 
                        [0.1-0.2j, 0.3, 0.05+0.1j],
                        [0.0, 0.05-0.1j, 0.2]], dtype=complex)
        
        traj = rk4_lvne_traj(H0, mu_x, mu_y, Ex, Ey, rho0, dt=0.1, steps=5, sample_stride=1)
        
        for i, rho in enumerate(traj):
            hermiticity_error = np.max(np.abs(rho - rho.conj().T))
            assert hermiticity_error < 1e-12, f"Step {i}: エルミート性誤差 = {hermiticity_error}"
...
class TestErrorHandling:
    ...
    def test_field_length_validation(self):
        ...
        with pytest.raises(ValueError):
            rk4_schrodinger(H0, mu_x, mu_y, np.array([1.0]), np.array([0.0]), 
                          psi0, dt=0.1)
...
class TestAlgorithmComparison:
    ...
    def test_rk4_vs_splitop_weak_field(self):
        ...
        # Split-Operator
        traj_splitop = splitop_schrodinger(H0, mu_x, mu_y, pol, Efield, psi0,
                                         dt, steps=(len(Ex)-1)//2)
