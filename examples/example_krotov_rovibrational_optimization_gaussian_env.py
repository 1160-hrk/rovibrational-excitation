...
# TARGET_STATE = (1, 1, -1)   # |v=1, J=1, M=1⟩ - xy偏光で可能な遷移
TARGET_STATE = (2, 0, 0)   # |v=1, J=1, M=1⟩ - xy偏光で可能な遷移

# 時間設定
TIME_TOTAL = 1000.0  # 最適化時間窓 [fs]
...
LAMBDA_A = 1e-20     # 電場制約パラメータ（適切な値に調整）
...
ENVELOPE_DURATION = TIME_TOTAL*4  # [fs] ガウシアンFWHM。None/<=0 で無効
...
class KrotovOptimizer:
    ...
    def calculate_field_update(self, psi_traj: np.ndarray, 
        ...
        for i in range(n_traj_steps):
            ...
            chi_i = chi_traj[i]
            # Krotov勾配: 2 * Im[⟨χ|μ|ψ⟩]
            grad_x = 2.0 * float(np.imag(np.vdot(chi_i, (mu_x @ psi_i))))
            grad_y = 2.0 * float(np.imag(np.vdot(chi_i, (mu_y @ psi_i))))
            # 形状関数（境界0）
    ...
    def optimize(self, lambda_a: float = LAMBDA_A, 
        ...
        initial_amplitude = 5e9  # 実用的電場スケールに調整 [V/m]
        
        # X偏光成分（σ_x遷移用）
        efield.add_dispersed_Efield(
            envelope_func=gaussian,
            duration=TIME_TOTAL/2,  # パルス幅を調整
            t_center=tc,
