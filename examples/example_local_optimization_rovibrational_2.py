...
# Time grid
TIME_TOTAL = 200000.0  # [fs]
DT_EFIELD = 0.1      # [fs]
SAMPLE_STRIDE = 1

# Local optimization parameters
GAIN = 3.75e19          # 局所制御則のゲイン（振幅スケール）
...
def plot_results(optimizer: LocalOptimizer, efield: ElectricField, psi_traj: np.ndarray,
    ...
    fmin: float = float(max(0.0, OMEGA_01 - 500.0))
    fmax: float = float(OMEGA_01 + 500.0)
    ...
    try:
        ...
        # Overlay R/P-branch transition lines from H0 (V→V+1, ΔJ=±1)
        try:
            eigenvalues = optimizer.hamiltonian.get_eigenvalues()  # rad/fs
            states = optimizer.basis.basis
    ...
    try:
        ...
        if 'fmin' not in locals() or 'fmax' not in locals():
            # スペクトル中心を OMEGA_01 付近と仮定して広めに設定
